...
public class ElevatorSubsystem extends SubsystemBase {
    ...
    public enum ElevatorPosition { // Values must be set in ElevatorConstants
        ...
        TESTDOWN(ElevatorConstants.test);
 ...
 //       encoder.setPosition(0); // Reset home position on startup
        // Auto-home on startup
        homeElevator();
    ...
    public void periodic() {
        ...
        SmartDashboard.putNumber("Periodic Pos", currentPos);
        SmartDashboard.putNumber("curStatePeriod", currentState.position);
        SmartDashboard.putNumber("goalStatePeriod", goalState.position);
        //double pidOutput = pidController.calculate(getHeightInches(), currentState.position);
        //SmartDashboard.putNumber("PID Output", pidOutput);
        SmartDashboard.putNumber("ff", calculateFeedForward(currentState));
        ...
        if (isHomed) {
            double pidOutput = pidController.calculate(getHeightInches(), currentState.position);
            double ff = 0;//calculateFeedForward(currentState);
          ...
          primaryMotor.set(outputPower);  //COMMENT OUT TO DISABLE ELEVATOR
        }
    ...
    */
    public void resetHomePosition() {
        encoder.setPosition(0);
        isHomed = true;
        System.out.println("Elevator home position reset.");
    }
